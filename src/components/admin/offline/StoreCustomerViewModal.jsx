...
const STATUS_CONFIG = {
  'Placed': { color: 'text-amber-500 bg-amber-50', icon: Clock },
  'Processing': { color: 'text-blue-500 bg-blue-50', icon: PackageCheck },
  'Ready': { color: 'text-indigo-500 bg-indigo-50', icon: Truck },
...
const StoreCustomerViewModal = ({ isOpen, onClose, customer }) => {
  ...
  useEffect(() => {
    const fetchUserOrders = async () => {
      ...
      try {
        ...
        if (customer.id) {
          promises.push(getDocs(query(ordersRef, where('customerId', '==', customer.id))));
        }
        if (customer.email) {
          promises.push(getDocs(query(ordersRef, where('customerEmail', '==', customer.email))));
        }
        ...
        console.error("Failed to fetch store customer orders", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const deliveredOrders = orders.filter(o => o.orderStatus === 'Completed' || o.orderStatus === 'Delivered').length;
  const cancelledOrders = orders.filter(o => ['Cancelled', 'Returned'].includes(o.orderStatus)).length;
  const totalSpent = orders.filter(o => !['Cancelled', 'Returned'].includes(o.orderStatus)).reduce((acc, curr) => {
    const total = curr.pricing?.grandTotal || curr.total || 0;
    const num = typeof total === 'number' ? total : Number((total || '').toString().replace(/[^0-9.-]+/g,""));
            ...
            <div className="px-8 py-6 flex items-center justify-between bg-white border-b border-gray-50 shrink-0">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <h2 className="text-xl font-bold text-gray-900 leading-none">Store Customer Profile</h2>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 text-gray-600">
                      ...
                      <span className="text-sm font-semibold text-gray-900">{customer.fullName || 'No Name'}</span>
                    </div>
                    <div className="flex items-center gap-3 text-gray-600">
                    ...
                    <div className="flex items-center gap-3 text-gray-600">
                      ...
                      <span className="text-sm font-medium">{customer.phone || 'No phone provided'}</span>
                    </div>
                    <div className="flex items-center gap-3 text-gray-600">
                      <MapPin size={16} className="text-gray-300" />
                      <span className="text-sm font-medium">{customer.address || 'No address found'}</span>
                    </div>
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-3">
                    <span className={`text-[11px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg ${
                        (customer.status || 'Active') === 'Active' ? 'text-[#1BAFAF] bg-[#eaf6f6]' : 'text-red-500 bg-red-50'
                      }`}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="bg-white rounded-xl p-3.5 shadow-sm border border-gray-100 hover:shadow-md transition-all group">
                  ...
                  <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider mb-0.5">Completed</p>
                ...
                <div className="bg-white rounded-xl p-3.5 shadow-sm border border-gray-100 hover:shadow-md transition-all group">
                  ...
                  <p className="text-lg font-black text-gray-900">{loading ? '...' : `₹${totalSpent.toFixed(2)}`}</p>
                </div>
              </div>
                              ...
                              <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td className="px-5 py-4">
                                  <span className="text-[13px] font-medium text-gray-900">
                                    {order.items ? order.items.reduce((sum, i) => sum + (i.quantity || i.qty || 1), 0) : (order.quantity || 1)} Item(s)
                                  </span>
                                </td>
                                <td className="px-5 py-4">
                                ...
                                <td className="px-5 py-4 text-right">
                                  <span className="text-[13px] font-black text-gray-900">₹{(order.pricing?.grandTotal || order.total || 0).toFixed(2)}</span>
                                </td>
                              </tr>
                            );
                          })}
...
export default StoreCustomerViewModal;
