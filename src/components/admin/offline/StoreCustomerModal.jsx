...
export default function StoreCustomerModal({ isOpen, onClose, customer = null }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (customer) {
        // Update
        await updateDoc(doc(db, 'storeCustomers', customer.id), {
          ...formData,
        ...
        <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-gray-50 to-white">
          <div>
            <h2 className="text-[20px] font-bold text-gray-900 tracking-tight">
              {customer ? 'Edit Store Customer' : 'Add Store Customer'}
            </h2>
            <p className="text-[12px] text-gray-400 font-medium">Customer details for offline store</p>
          </div>
          <button 
            onClick={onClose}
