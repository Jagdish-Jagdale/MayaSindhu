...
const FilterAccordion = ({ title, children, isOpen, onToggle }) => {
  return (
    <div className="border-b border-gray-200/60 py-5">
      <button
        onClick={onToggle}
        className="flex items-center justify-between w-full text-[13px] font-fashion font-bold tracking-tight uppercase text-brand-black hover:text-brand-orange transition-colors group"
      >
        <span>{title}</span>
        <ChevronDown 
          size={14} 
            ...
            {subranges.map((range) => {
              ...
              return (
                <Link 
                  key={range.id} 
                  to={range.fullPath}
                  className={`flex items-center justify-between group py-1 text-[11px] font-fashion font-bold uppercase tracking-tight transition-all ${
