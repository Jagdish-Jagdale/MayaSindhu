...
export default function Admins() {
  ...
  const [formData, setFormData] = useState({ 
    id: null, name: '', email: '', password: '', confirmPassword: '', role: 'Admin', status: 'Active',
    isEcommerceAdmin: true, isOfflineStoreAdmin: false
  });
  ...
  const openEditModal = (admin) => {
    ...
    setFormData({ 
      ...
      isOfflineStoreAdmin: admin.isOfflineStoreAdmin || false
    });
    setShowPassword(false);
    setShowConfirmPassword(false);
    setIsModalOpen(true);
  ...
  const handleSave = async (e) => {
    ...
    // Admin Access Scope Validation
    if (!formData.isEcommerceAdmin && !formData.isOfflineStoreAdmin) {
      return toast.error("Please select at least one Admin Access Scope");
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(`Admin "${adminToDelete.name || adminToDelete.email}" removed`);
  ...
  if (loading) {
    return (
      <div className="h-[60vh] w-full flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-[14px] font-medium text-gray-400">Loading administrative hierarchy...</p>
      </div>
    );
  }
      ...
      <div className="space-y-2 py-2">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[12px] text-gray-400 font-medium font-inter">Manage privileged access, system administrators, and roles.</p>
          </div>
          <button 
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-gray-50 bg-gray-50/30">
              ...
              <th className="px-6 py-4 text-left text-[14px] font-bold text-gray-500">Access Scope</th>
            ...
            {filteredAdmins.length > 0 ? (
              filteredAdmins.map((admin, index) => (
                <tr key={admin.id} className="hover:bg-gray-50 group transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-[13px] text-gray-900 font-bold">{admin.email?.toLowerCase()}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`text-[11px] font-bold border uppercase tracking-wider px-2.5 py-1 rounded-lg ${
                      admin.role === 'Super Admin' ? 'bg-[#1BAFAF]/10 text-[#1BAFAF] border-[#1BAFAF]/20' : 'bg-gray-100 text-gray-600 border-gray-200'
                    }`}>
                      {admin.role}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex gap-1.5 flex-wrap max-w-[140px]">
                      {admin.isEcommerceAdmin && <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 bg-blue-50 text-blue-600 rounded">E-Com</span>}
                      {admin.isOfflineStoreAdmin && <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 bg-purple-50 text-purple-600 rounded">Offline</span>}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <h2 className="text-[16px] font-bold text-gray-900">
                {modalMode === 'add' ? 'Add New Administrator' : 'Update Administrator'}
              </h2>
              <button 
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div className="pt-2">
                <label className="block text-[12px] font-semibold text-gray-600 mb-2 ml-1">Admin Access Scope</label>
                <div className="flex items-center gap-6 bg-gray-50 border border-gray-100 p-3.5 rounded-xl">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <div className={`w-5 h-5 rounded flex items-center justify-center border transition-colors ${formData.isEcommerceAdmin ? 'bg-[#1BAFAF] border-[#1BAFAF]' : 'bg-white border-gray-300 group-hover:border-[#1BAFAF]'}`}>
                      {formData.isEcommerceAdmin && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>}
                    </div>
                    <input 
                      type="checkbox" 
                      className="hidden" 
                      checked={formData.isEcommerceAdmin} 
                      onChange={(e) => setFormData({...formData, isEcommerceAdmin: e.target.checked})} 
                    />
                    <span className="text-[13px] font-semibold text-gray-800">E-Commerce Admin</span>
                  </label>
                  
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <div className={`w-5 h-5 rounded flex items-center justify-center border transition-colors ${formData.isOfflineStoreAdmin ? 'bg-[#1BAFAF] border-[#1BAFAF]' : 'bg-white border-gray-300 group-hover:border-[#1BAFAF]'}`}>
                      {formData.isOfflineStoreAdmin && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>}
                    </div>
                    <input 
                      type="checkbox" 
                      className="hidden" 
                      checked={formData.isOfflineStoreAdmin} 
                      onChange={(e) => setFormData({...formData, isOfflineStoreAdmin: e.target.checked})} 
                    />
                    <span className="text-[13px] font-semibold text-gray-800">Offline Store Admin</span>
                  </label>
                </div>
              </div>
              ...
              <div className="mt-8 flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
                ...
                >
                  ...
                  {isSaving ? 'Processing...' : modalMode === 'add' ? 'Create Administrator' : 'Update'}
                </button>
              </div>
            </form>
          </div>
